...
Within the `src` directory, the deephaven_plugin_audio directory contains the Python code, and the `js` directory contains the JavaScript code.

The Python files have the following structure:
`deephaven_plugin_audio_object.py` defines a simple Python class that can send messages to the client.
`deephaven_plugin_audio_type.py` defines the Python type for the plugin (which is used for registration) and a simple message stream.
...
`DeephavenPluginAudioPlugin.ts` registers the plugin with Deephaven.
`DeephavenPluginAudioView.tsx` defines the plugin panel and message handling.
...
pip install dist/deephaven_plugin_audio-0.0.1.dev0-py3-none-any.whl
deephaven server
```
...
```sh
pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ deephaven_plugin_audio
```
...
```sh
pip install deephaven_plugin_audio
```
