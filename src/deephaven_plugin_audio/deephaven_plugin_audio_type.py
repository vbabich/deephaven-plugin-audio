...
from .deephaven_plugin_audio_object import DeephavenPluginAudioObject

# Create a custom message stream to send messages to the client

class DeephavenPluginAudioMessageStream(MessageStream):
    """
...
# The value returned by name() should match supportedTypes in DeephavenPluginAudioPlugin.ts
class DeephavenPluginAudioType(BidirectionalObjectType):
    """
    ...
    @property
    def name(self) -> str:
        return "DeephavenPluginAudio"

    def is_type(self, obj: Any) -> bool:
        return isinstance(obj, DeephavenPluginAudioObject)

    def create_client_connection(
        self, obj: object, connection: MessageStream
    ) -> MessageStream:
        message_stream = DeephavenPluginAudioMessageStream(obj, connection)
